...
public class SpawnManager : NetworkBehaviour 
{
    ...
    /// <summary>
    /// Method DespawnOldPowerUps
    /// This method get the spawned powerUps and despawn all of these
    /// </summary>
    public void DespawnOldPowerUps()
    {
        var nObjets = NetworkManager.Singleton.SpawnManager.SpawnedObjects;
        List<NetworkObject> nObjToDespawn = new List<NetworkObject>();
        foreach (KeyValuePair<ulong, NetworkObject> no in nObjets)
        {
            if(no.Value.CompareTag("PowerUp")) nObjToDespawn.Add(no.Value);
        }

        foreach (var pUp in nObjToDespawn)
        {
            pUp.Despawn();
        }
    }
